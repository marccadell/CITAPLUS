...
const CreateAppointment = () => {
  ...
  const handleDateChange = async (e) => {
    ...
    if (selectedDateTime < currentDate) {
      ...
      showToastError('Fecha No Disponible');
    ...
    if (!querySnapshot.empty) {
      ...
      showToastError('Esta fecha ya está programada en otra cita');
      setIsAvailable(false);
    } else {
      setErrorMessage('');
      setIsAvailable(true);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!patientName || !appointmentDate || !diagnostico || !centroMedico || !servicio) {
      showToastError('Por favor, verifique que todos los campos sean correctos');
      return;
    }

    if (!isAvailable) {
      showToastError('No se puede crear la cita debido a la disponibilidad de la fecha.');
      return;
    }
    ...
    try {
      ...
      showToastSuccess('Cita creada exitosamente');
    } catch (error) {
      console.error('Error al crear la cita:', error);
      showToastError('Hubo un error al crear la cita. Por favor, inténtalo de nuevo.');
    }
  };
