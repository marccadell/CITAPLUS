...
const PatientsList = () => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, 'patients'), (snapshot) => {
      const patientsList = snapshot.docs.map(doc => ({
        id: doc.id,  // This is the document ID in the 'patients' collection
        ...doc.data(),
      }));
  ...
  const fetchAppointments = async (patientId) => {
    try {
      const now = Timestamp.now();  // Get current time as Firestore Timestamp
      const q = query(
        collection(db, 'appointments'),
        where('patientId', '==', patientId),  // Ensure that patientId matches the field in appointments
        where('appointmentDate', '<', now) // Make sure appointmentDate is a Timestamp
      );

      console.log('Fetching appointments for patientId:', patientId); // Debugging

      const querySnapshot = await getDocs(q);

      // Verify if any documents were found
      ...
      console.log('Appointments list:', appointmentsList); // Debugging
              ...
              <td>
                <button
                  ...
                  onClick={() => {
                    ...
                    fetchAppointments(patient.id); // Fetch appointments for the selected patient
                  }}
                >
                  Historial
                </button>
